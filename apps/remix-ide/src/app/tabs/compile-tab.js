...
class CompileTab extends CompilerApiMixin(ViewPlugin) {
  ...
  setCompilerConfig(settings) {
    ...
    this.call(
      "notification",
      "toast",
      compilerConfigChangedToastMsg(this.currentRequest.from, value)
    );
  }
  ...
  compile(fileName) {
    if (!isNative(this.currentRequest.from))
      this.call(
        "notification",
        "toast",
        compileToastMsg(this.currentRequest.from, fileName)
      );
  ...
  async onActivation() {
    ...
    try {
      ...
      if (error.message !== "Error: No such file or directory No file selected")
        throw error;
    }
  }
