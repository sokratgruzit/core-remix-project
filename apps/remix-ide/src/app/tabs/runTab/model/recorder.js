...
class Recorder extends Plugin {
  constructor(blockchain) {
    ...
    this.blockchain.event.register(
      "initiatingTransaction",
      (timestamp, tx, payLoad) => {
        ...
        if (this.data._listen) {
          ...
          if (!to) {
            ...
            var keccak = ethutil.bufferToHex(
              ethutil.keccakFromString(JSON.stringify(abi))
            );
            record.abi = keccak;
            ...
            if (
              ...
              Object.keys(record.linkReferences).length
            ) {
          ...
          for (var p in record.parameters) {
            ...
            if (thistimestamp)
              record.parameters[p] = `created{${thistimestamp}}`;
          }
      ...
      async (tx, index, cb) => {
        if (liveMode && tx.record.type === "constructor") {
          ...
          const data = await this.call(
            ...
            tx.record.contractName
          );
          tx.record.bytecode = data.artefact.evm.bytecode.object;
          const updatedABIKeccak = ethutil.bufferToHex(
            ethutil.keccakFromString(JSON.stringify(data.artefact.abi))
          );
        ...
        if (
          ...
          for (var k in linkReferences) {
            ...
            tx.record.bytecode = format.linkLibraryStandardFromlinkReferences(
              k,
              link.replace("0x", ""),
              tx.record.bytecode,
              tx.record.linkReferences
            );
          }
        }
        /* Encode params */
          ...
          try {
            tx.record.parameters.forEach((value, index) => {
              ...
              for (var timestamp in this.data._createdContractsReverse) {
                value = value.replace(
                  new RegExp("created\\{" + timestamp + "\\}", "g"),
                  this.data._createdContractsReverse[timestamp]
                );
              }
            ...
            return alertCb(
              "cannot resolve input parameters " +
                JSON.stringify(tx.record.parameters) +
                ". Execution stopped at " +
                index
            );
          }
        }
        var data = format.encodeData(
          fnABI,
          tx.record.parameters,
          tx.record.bytecode
        );
        if (data.error) {
          alertCb(
