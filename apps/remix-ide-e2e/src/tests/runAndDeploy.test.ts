...
module.exports = {
  ...
  "Should load run and deploy tab #group1 #group2": function (
  ...
  "Should load run and deploy tab and check value validation #group1":
  ...
  "Should sign message using account key #group2": function (
    browser: NightwatchBrowser
  ) {
    browser
      ...
      .waitForElementVisible(
        '*[data-id="udappNotifyModalDialogModalBody-react"]',
        12000
      )
  ...
  "Should deploy contract on JavascriptVM #group3": function (
    browser: NightwatchBrowser
  ) {
    browser
  ...
  "Should show and update balance for deployed contract on JavascriptVM #group3":
    function (browser: NightwatchBrowser) {
      let instanceAddress;
      browser
        .waitForElementVisible('*[data-id="remixIdeSidePanel"]')
        ...
        .waitForElementVisible(
          '*[data-id="Deploy - transact (payable)"]',
          45000
        )
        ...
        .getAddressAtPosition(1, (address) => {
          ...
          browser
            .waitForElementVisible(
              `#instance${instanceAddress} [data-id="instanceContractBal"]`
            )
            .assert.containsText(
              `#instance${instanceAddress} [data-id="instanceContractBal"]`,
              "Balance: 0.000000000000000111 ETH"
            )
            .clickFunction("sendSomeEther - transact (not payable)", {
              types: "uint256 num",
              values: "2",
            })
            .pause(1000)
            .assert.containsText(
              `#instance${instanceAddress} [data-id="instanceContractBal"]`,
              "Balance: 0.000000000000000109 ETH"
            );
        });
    },

  "Should run low level interaction (fallback function) #group3": function (
    browser: NightwatchBrowser
  ) {
    browser
      .waitForElementPresent('*[data-id="remixIdeSidePanel"]')
  ...
  "Should connect to Goerli Test Network using MetaMask": !function (
    browser: NightwatchBrowser
  ) {
    browser
      .waitForElementPresent('*[data-id="remixIdeSidePanel"]')
  ...
  "Should deploy contract on Goerli Test Network using MetaMask": !function (
    browser: NightwatchBrowser
  ) {
    browser
  ...
  "Should connect to CORE Main Network using MetaMask": !function (
    browser: NightwatchBrowser
  ) {
    browser
      .waitForElementPresent('*[data-id="remixIdeSidePanel"]')
      ...
      .click(".network-indicator__down-arrow")
      .useXpath()
      .click("//span[text()='Main CORE Network']")
      .useCss()
      .switchBrowserTab(0)
      .refreshPage()
      .waitForElementVisible('*[data-id="remixIdeIconPanel"]', 10000)
  ...
  "Should deploy contract on CORE Main Network using MetaMask": !function (
    browser: NightwatchBrowser
  ) {
    browser
      .waitForElementPresent('*[data-id="runTabSelectAccount"] option')
  ...
  "Call web3.eth.getAccounts() using Injected Provider (Metamask)": !function (
    browser: NightwatchBrowser
  ) {
    browser
