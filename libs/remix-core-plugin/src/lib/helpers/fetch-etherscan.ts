export const fetchContractFromEtherscan = async (
  plugin,
  network,
  contractAddress,
  targetPath,
  ...
  if (!key)
    etherscanKey = await plugin.call(
      "config",
      "getAppParameter",
      ...
      if (data.message === "OK" && data.status === "1") {
        if (data.result.length) {
          if (data.result[0].SourceCode === "")
            throw new Error(
              `contract not verified on Etherscan ${network.name} network`
            );
          if (data.result[0].SourceCode.startsWith("{")) {
            data.result[0].SourceCode = JSON.parse(
              data.result[0].SourceCode.replace(/(?:\r\n|\r|\n)/g, "")
                .replace(/^{{/, "{")
                .replace(/}}$/, "}")
            );
          }
        ...
        throw new Error("unable to retrieve contract data " + data.message);
    } catch (e) {
      throw new Error("unable to retrieve contract data: " + e.message);
    }
  } else
    throw new Error(
      "unable to try fetching the source code from etherscan: etherscan access token not found. please go to the Remix settings page and provide an access token."
    );
  ...
  if (typeof data.result[0].SourceCode === "string") {
    ...
    data.result[0].SourceCode &&
    typeof data.result[0].SourceCode == "object"
  ) {
    const sources = data.result[0].SourceCode.sources;
    for (let [file, source] of Object.entries(sources)) {
      // eslint-disable-line
      file = file.replace("browser/", ""); // should be fixed in the Apeirogon IDE end.
