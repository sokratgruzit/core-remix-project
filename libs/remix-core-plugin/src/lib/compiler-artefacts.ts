...
export class CompilerArtefacts extends Plugin {
  ...
  onActivation() {
    const saveCompilationPerFileResult = (
      file,
      source,
      languageVersion,
      data,
      ...
      this.compilersArtefactsPerFile[file] = new CompilerAbstract(
        languageVersion,
        data,
        source,
        input
    ...
    this.on(
      ...
      (file, source, languageVersion, data, input, version) => {
        this.compilersArtefacts.__last = new CompilerAbstract(
          languageVersion,
          data,
          source,
          input
  ...
  /**
   * Get compilation output for contracts compiled during a session of Apeirogon IDE
  ...
  /**
   * filter compilation output for contracts compiled during a session of Apeirogon IDE
   * @returns compilatin output
   */
  filterAllContractDatas(filter) {
    ...
    Object.keys(this.compilersArtefactsPerFile).map((targetFile) => {
      const contracts =
        this.compilersArtefactsPerFile[targetFile].getContracts();
  ...
  _getAllContractArtefactsfromOutput(compilerOutput, contractName) {
    ...
    for (const filename in compilerOutput) {
      if (Object.keys(compilerOutput[filename]).includes(contractName))
        contractArtefacts[filename + ":" + contractName] =
   ...
   */
  async _populateAllContractArtefactsFromFE(
    path,
    contractName,
    contractArtefacts
  ) {
    ...
    if (dirList && dirList.length) {
      for (const dirPath of dirList) {
        ...
        if (
          dirPath === path + "/artifacts" &&
          (await this.call("fileManager", "exists", dirPath + "/build-info"))
        ) {
          const buildFileList = await this.call(
            "fileManager",
            "fileList",
            dirPath + "/build-info"
          );
          ...
          for (const buildFile of buildFileList) {
            ...
            const artefacts = this._getAllContractArtefactsfromOutput(
              compilerOutput,
              contractName
            );
          ...
          await this._populateAllContractArtefactsFromFE(
            dirPath,
            contractName,
            contractArtefacts
          );
  ...
  async getArtefactsByContractName(name) {
    ...
    if (name.includes(":")) {
      ...
      if (
        Object.keys(contractsDataByFilename).includes(filename) &&
        contractsDataByFilename[filename][contract]
      )
        ...
        if (allContractsData[fullyQualifiedName])
          ...
          throw new Error(
            `Could not find artifacts for ${fullyQualifiedName}. Compile contract to generate artifacts.`
          );
      }
    } else {
      const contractName = name;
      const contractArtefacts = this._getAllContractArtefactsfromOutput(
        contractsDataByFilename,
        contractName
      );
      ...
      if (keys.length === 1)
        ...
        throw new Error(
          `Could not find artifacts for ${contractName}. Compile contract to generate artifacts.`
        );
    }
  }
