...
export function ContractDropdownUI(props: ContractDropdownProps) {
  ...
  const [loadedContractData, setLoadedContractData] =
    useState<ContractData>(null);
  const [constructorInterface, setConstructorInterface] =
    useState<FuncABI>(null);
  ...
  const enableAtAddress = (enable: boolean) => {
    if (enable) {
      ...
      setAtAddressOptions({
        disabled: true,
        title: loadedAddress
          ? "Compile a *.sol file or select a *.abi file."
          : "To interact with a deployed contract, enter its address and compile its source *.sol file (with the same compiler settings) or select its .abi file in the editor. ",
      });
    }
  };
  ...
  const enableContractNames = (enable: boolean) => {
    if (enable) {
      setContractOptions({
        disabled: false,
        title:
          "Select a compiled contract to deploy or to use with At Address.",
      });
    } else {
      setContractOptions({
        disabled: true,
        title:
          loadType === "sol"
            ...
            : "When there is a compiled .sol file, the choice of contracts to deploy or to use with AtAddress is made here.",
      });
    }
  };
  ...
  const checkSumWarning = () => {
    return (
      <span>
        ...
        <br />A checksummed address is an address that contains uppercase
        ...
        .
        <br />
        Checksummed addresses are meant to help prevent users from sending
        transactions to the wrong address.
      </span>
    );
  };
  ...
  const isOverSizePrompt = () => {
    return (
      <div>
        Contract creation initialization returns data with length of more than
        24576 bytes. The deployment will likely fails. <br />
        More info:{" "}
        <a
  ...
  return (
    <div className="udapp_container" data-id="contractDropdownContainer">
          ...
          >
            ...
            >
              <i
                ...
                className="fa fa-refresh mr-2 mt-2"
                aria-hidden="true"
              ></i>
            </button>
          </CustomTooltip>
      ...
      <div className="font-12 text-dark">
        <div className="udapp_deployDropdown font-12 text-dark">
          {(
        ...
        <div className="udapp_button udapp_atAddressSect ">
          ...
          >
            ...
            >
              <button
                className="udapp_atAddress btn font-14 text-dark"
