...
function PermisssionsSettings() {
  ...
  const [permissions, setPermissions] = useLocalStorage<PluginPermissions>(
    "plugins/permissions",
  ...
  const openModal = () => {
    const currentValue = JSON.parse(
      window.localStorage.getItem("plugins/permissions") || "{}"
    );
  ...
  const handleCheckboxClick = (
    ...
    setPermissions((permissions) => {
      permissions[targetPlugin][funcName][pluginName].allow =
        !permissions[targetPlugin][funcName][pluginName].allow;
      return permissions;
    });
  };

  function clearFunctionPermission(
    targetPlugin: string,
    funcName: string,
    pluginName: string
  ) {
    ...
    return (
      <>
        {Object.keys(permissions[targetPlugin]).map((funcName) => {
          return Object.keys(permissions[targetPlugin][funcName]).map(
            (pluginName, index) => (
              <div
          ...
          <div className="p-2">
            {Object.keys(permissions).map((targetPlugin) => (
              <div key={`container-${targetPlugin}`}>
                <RenderPluginHeader
                  key={`header-${targetPlugin}`}
                  headingName={targetPlugin}
                />
                <RenderPermissions
                  key={`permissions-${targetPlugin}`}
