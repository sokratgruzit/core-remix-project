...
export const addInputField = async (
  ...
  cb?: (
    err: Error,
    result?: string | number | boolean | Record<string, any>
  ) => void
) => {
...
export const createWorkspaceTemplate = async (
  ...
  if ((await workspaceExists(workspaceName)) && template === "remixDefault")
...
export const loadWorkspacePreset = async (
  template: WorkspaceTemplate = "remixDefault",
  opts?
) => {
  const workspaceProvider = plugin.fileProviders.workspace;
  ...
  switch (template) {
    case "code-template":
      ...
      try {
        ...
        if (params.code) {
          ...
          path =
            "contract-" +
            hash.replace("0x", "").substring(0, 10) +
            (params.language && params.language.toLowerCase() === "yul"
              ? ".yul"
              : ".sol");
        ...
        if (params.url) {
          ...
          try {
            ...
            if (
              content.language &&
              content.language === "Solidity" &&
              content.sources
            ) {
              const standardInput: JSONStandardInput =
                content as JSONStandardInput;
              for (const [fname, source] of Object.entries(
    ...
    case "gist-template":
      ...
      try {
        ...
        plugin.fileManager.setBatchFiles(
          obj,
          "workspace",
          true,
          (errorLoadingFile) => {
            if (errorLoadingFile) {
              dispatch(
                displayNotification(
                  "",
                  errorLoadingFile.message || errorLoadingFile,
                  "OK",
                  null,
                  () => {},
                  null
...
export const renameWorkspaceFromProvider = async (
  oldName: string,
  workspaceName: string
) => {
...
const deleteWorkspaceFromProvider = async (workspaceName: string) => {
  ...
  await plugin.fileManager.closeAllFiles();
  await plugin.fileProviders.browser.remove(
    workspacesPath + "/" + workspaceName
  );
  await plugin.setWorkspaces(await getWorkspaces());
};
...
export const switchToWorkspace = async (name: string) => {
  ...
  if (name === LOCALHOST) {
    ...
    plugin.call(
      "notification",
      "toast",
      `No workspace found! Creating default workspace ....`
    );
...
export const uploadFile = async (
  ...
  [...target.files].forEach(async (file) => {
    ...
    const name =
      targetFolder === "/" ? file.name : `${targetFolder}/${file.name}`;
...
export const getWorkspaces = async ():
  ...
  try {
    const workspaces: {
      ...
      plugin.fileProviders.browser.resolveDirectory(
        "/" + workspacesPath,
        (error, items) => {
          if (error) {
            return reject(error);
            ...
            Object.keys(items)
              ...
              .map(async (folder) => {
                const isGitRepo: boolean =
                  await plugin.fileProviders.browser.exists(
                    "/" + folder + "/.git"
                  );
    ...
    promise
      .then(async () => {
        ...
        const branches = await getGitRepoBranches(
          workspacesPath + "/" + repoName
        );

        dispatch(setCurrentWorkspaceBranches(branches));
        const currentBranch = await getGitRepoCurrentBranch(
          workspacesPath + "/" + repoName
        );
...
export const getGitRepoBranches = async (workspacePath: string) => {
  ...
  const branches: { remote: any; name: string }[] = await plugin.call(
    "dGitProvider",
    "branches",
    { ...gitConfig }
  );
...
export const getGitRepoCurrentBranch = async (workspaceName: string) => {
  ...
  const currentBranch: string = await plugin.call(
    "dGitProvider",
    "currentbranch",
    { ...gitConfig }
  );
        ...
        plugin
          ...
          .then(async () => {
            ...
            const workspacesPath =
              plugin.fileProviders.workspace.workspacesPath;
            const workspaceName = plugin.fileProviders.workspace.workspace;
            const branches = await getGitRepoBranches(
              workspacesPath + "/" + workspaceName
            );
