...
export const listenOnPluginEvents = (filePanelPlugin) => {
  ...
  plugin.on(
    "fileDecorator",
    "fileDecoratorsChanged",
    async (items: fileDecoration[]) => {
...
export const listenOnProviderEvents =
  (provider) => (reducerDispatch: React.Dispatch<any>) => {
      ...
      (path: string, content: string) => {
        ...
        if (
          ...
          dispatch(
            displayNotification(
              path + " changed",
              "This file has been changed outside of Apeirogon IDE.",
...
const setFileDecorators = async (
  items: fileDecoration[],
  cb?: (
    err: Error,
    result?: string | number | boolean | Record<string, any>
  ) => void
