...
const basicWorkspaceInit = async (
  workspaces: { name: string; isGitRepo: boolean }[],
  workspaceProvider
) => {
  if (workspaces.length === 0) {
    ...
    dispatch(
      setCurrentWorkspace({ name: "default_workspace", isGitRepo: false })
    );
...
export const initWorkspace =
  (filePanelPlugin) => async (reducerDispatch: React.Dispatch<any>) => {
    if (filePanelPlugin) {
      ...
      if (params.gist) {
        ...
        dispatch(
          setCurrentWorkspace({ name: "gist-sample", isGitRepo: false })
        );
        ...
        dispatch(
          setCurrentWorkspace({ name: "code-sample", isGitRepo: false })
        );
        const filePath = await loadWorkspacePreset("code-template");
        plugin.on(
          "editor",
          "editorMounted",
          async () => await plugin.fileManager.openFile(filePath)
        );
        ...
        if (params.address.startsWith("0x") && params.address.length === 42) {
          ...
          plugin.call(
            "notification",
            "toast",
            `Looking for contract(s) verified on different networks of Etherscan for contract address ${contractAddress} .....`
          );
          let data;
          let count = 0;
          try {
            let etherscanKey = await plugin.call(
              "config",
              "getAppParameter",
              "etherscan-access-token"
            );
            if (!etherscanKey)
              etherscanKey = "2HKUX5ZVASZIKWJM8MIQVCRUVZ6JAWT531";
            const networks = [
              { id: 1, name: "mainnet" },
              { id: 3, name: "ropsten" },
              { id: 4, name: "rinkeby" },
            ...
            for (const network of networks) {
              ...
              try {
                data = await fetchContractFromEtherscan(
                  plugin,
                  network,
                  contractAddress,
                  target,
              ...
              plugin.setWorkspace({ name: workspaceName, isLocalhost: false });
              dispatch(
                setCurrentWorkspace({ name: workspaceName, isGitRepo: false })
              );
              count = count + Object.keys(data.compilationTargets).length;
              for (filePath in data.compilationTargets)
                await workspaceProvider.set(
                  filePath,
                  data.compilationTargets[filePath]["content"]
                );
            }
            plugin.on(
              "editor",
            ...
            plugin.call(
              "notification",
              "toast",
              `Added ${count} verified contract${
                count === 1 ? "" : "s"
              } from ${foundOnNetworks.join(",")} network${
                foundOnNetworks.length === 1 ? "" : "s"
              } of Etherscan for contract address ${contractAddress} !!`
            );
          } catch (error) {
            await basicWorkspaceInit(workspaces, workspaceProvider);
          }
        ...
        const index = workspaces.findIndex(
          (element) => element.name == localStorage.getItem("currentWorkspace")
        );
        if (index !== -1) {
...
export const publishToGist = async (path?: string, type?: string) => {
  ...
  try {
    ...
    if (!accessToken) {
      ...
      const description =
        "Created using remix-ide: Realtime Apeirogon Contract Compiler and Runtime. \n Load this file by pasting this gists URL or ID at https://remix.ethereum.org/#version=" +
        params.version +
        "&optimize=" +
        params.optimize +
        "&runs=" +
...
export const createNewFile = async (path: string, rootDir: string) => {
  ...
  if (!createFile) {
    ...
    const path =
      newName.indexOf(rootDir + "/") === 0
        ? newName.replace(rootDir + "/", "")
        : newName;
...
export const setFocusElement = async (
...
export const createNewFolder = async (path: string, rootDir: string) => {
  ...
  path =
    path.indexOf(rootDir + "/") === 0 ? path.replace(rootDir + "/", "") : path;
...
export const deletePath = async (path: string[]) => {
  ...
  for (const p of path) {
    try {
      ...
      dispatch(
        displayPopUp(`Failed to remove ${isDir ? "folder" : "file"} ${p}.`)
      );
    }
  }
};
...
export const handleClickFile = async (
  path: string,
...
const packageGistFiles = async (directory) => {
  ...
  return new Promise((resolve, reject) => {
    ...
    if (isFile) {
      ...
      try {
        (async () => {
          await workspaceProvider.copyFolderToJson(
            directory,
            ({ path, content }) => {
              if (/^\s+$/.test(content) || !content.length) {
                content =
...
export const moveFile = async (src: string, dest: string) => {
  ...
  try {
    ...
    dispatch(
      displayPopUp(
        "Oops! An error ocurred while performing moveFile operation." + error
      )
    );
  }
};
...
export const moveFolder = async (src: string, dest: string) => {
  ...
  try {
    ...
    dispatch(
      displayPopUp(
        "Oops! An error ocurred while performing moveDir operation." + error
      )
    );
  }
};
