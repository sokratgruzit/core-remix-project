...
export const FileExplorer = (props: FileExplorerProps) => {
  const { name, contextMenuItems, removedContextMenuItems, files, fileState } =
    props;
  ...
  const getFocusedFolder = () => {
    if (props.focusElement[0]) {
      if (props.focusElement[0].type === "folder" && props.focusElement[0].key)
        return props.focusElement[0].key;
      else if (
        props.focusElement[0].type === "gist" &&
        props.focusElement[0].key
      )
        return props.focusElement[0].key;
      else if (
        props.focusElement[0].type === "file" &&
        props.focusElement[0].key
      )
        return extractParentFromKey(props.focusElement[0].key)
          ? extractParentFromKey(props.focusElement[0].key)
          : ROOT_PATH;
  ...
  const editModeOn = (path: string, type: string, isNew = false) => {
    if (props.readonly)
      return props.toast("Cannot write/modify file system in read only mode.");
    setState((prevState) => {
      return {
        ...prevState,
