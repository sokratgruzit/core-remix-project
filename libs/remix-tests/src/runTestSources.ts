...
export class UnitTestRunner {
  ...
  async runTestSources(
    ...
    newCompilerConfig: CompilerConfiguration,
    testCallback,
    resultCallback,
    deployCb: any,
    finalCallback: any,
    ...
    async.waterfall(
      [
        ...
        function determineTestContractsToRun(
          ...
          for (const filename in compilationResult) {
            ...
            Object.keys(compilationResult[filename]).forEach((contractName) => {
              contractsToTestDetails.push(
                compilationResult[filename][contractName]
              );
        ...
        (
          contractsToTest: string[],
          contractsToTestDetails: any[],
          contracts: any,
          next
        ) => {
          let totalPassing = 0;
          ...
          async.eachOfLimit(
            ...
            (
              ...
              const fileAST: AstNode =
                sourceASTs[contracts[contractName]["filename"]];
              runTest(
                contractName,
                contracts[contractName],
                contractsToTestDetails[index],
                fileAST,
                { accounts: this.testsAccounts, web3: this.web3 },
                _testCallback,
                (err, result) => {
                  if (err) {
                    return cb(err);
